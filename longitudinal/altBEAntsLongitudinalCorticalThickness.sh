...
note: paths for brainExtraction*pl half-hardcoded: different directory not an option
...
Optional arguments:
     ...
     -C:  alternative brain extraction          If 1, then runs newer brain extraction on the single subject template during 
						antsCorticalThickness (instead of antsBrainExtraction). Should produce a tighter 
 						brain mask that includes less CSF and (ideally) less meninges. Requires -t specified.
						(Default = 0)  
USAGE
    exit 1
}
...
echoParameters() {
    ...
    Other parameters:
      ...
      cook brain extration    = ${DO_COOK_BRAINEXTRACTION}
...
DO_COOK_BRAINEXTRACTION=0


#############
# HARD CODE IN ORIGINATOR, but put an option here 
#############
ORIGINATE=1
...
if [[ $# -lt 3 ]] ; then
  ...
  while getopts "a:b:c:C:d:e:f:g:h:j:k:l:m:n:o:p:q:r:s:t:u:v:x:w:y:z:" OPT
    do
      case $OPT in
          a)
       ...
       ;;
          C) #brain extraction 
       DO_COOK_BRAINEXTRACTION=$OPTARG
       ;;
          d) #dimensions
       DIMENSION=$OPTARG
...
if [[ $DO_COOK_BRAINEXTRACTION -gt 0 && ! -f ${REGISTRATION_TEMPLATE} ]];
  then
    echo "Alternative brain extraction requires template for registration, and is not properly specified."
    exit 1
  fi
...
# 
if [[ $DO_COOK_BRAINEXTRACTION -eq 1 ]];
  then 
    SKIP_BE=1
  else  
    SKIP_BE=0
...
if [[ ${#ANATOMICAL_IMAGES[@]} -eq ${NUMBER_OF_MODALITIES} ]];
  then
    ...
    #      then
    #        # Phil's BE 
    #        COOK_BRAINEXTRACTION_SEGMENTATION_PRIOR=`echo ${SEGMENTATION_PRIOR} | sed 's/\%d/*/g'`

    #        logCmd ${bindir}/brainExtractionRegistration.pl \
    #          --input ${ANATOMICAL_IMAGES[1]} \
    #          --template ${BRAIN_TEMPLATE} \
    #          --template-brain-mask ${REGISTRATION_TEMPLATE} \
    #          --template-reg-mask ${EXTRACTION_REGISTRATION_MASK} \
    #          --bias-correct 1 \
    #          --laplacian-sigma 1 \
    #          --seg-priors ${COOK_BRAINEXTRACTION_SEGMENTATION_PRIOR} \
    #          --float 1 \
    #          --output-root ${OUTPUT_PREFIX}_initBEreg

    #        logCmd ${bindir}/brainExtractionSegmentation.pl \
    #          --input ${ANATOMICAL_IMAGES[1]} \
    #          --initial-brain-mask ${OUTPUT_PREFIX}_initBEregBrainMask.nii.gz \
    #          --bias-correct 1 \
    #          --erosion-radius 0 \
    #          --dilation-radius 2 \
    #          --include-initial-classes 4 5 6 \ 
    #          --output-root ${OUTPUT_PREFIX}_kMeans_

         #   logCmd cp ${OUTPUT_PREFIX}_kMeans_BrainMask.nii.gz ${OUTPUT_PREFIX}_BrainExtractionMask.nii.gz
    #        logCmd ${ANTSPATH}/CopyImageHeaderInformation ${ANATOMICAL_IMAGES[1]} ${OUTPUT_PREFIX}_kMeans_BrainMask.nii.gz ${OUTPUT_PREFIX}_BrainExtractionMask.nii.gz 1 1 1 
	 #   logCmd rm ${OUTPUT_PREFIX}_kMeans_BrainSegmentation.nii.gz

	#    logCmd rm ${OUTPUT_PREFIX}_initBEreg*
...
if [[ ! -f $SINGLE_SUBJECT_TEMPLATE ]];
  ...
  if [[ $ORIGINATE -eq 1 ]];
    then  # Modify origins to avoid problems with SST drifting out of FOV

      R_LIBS=/data/grossman/FMRI/software/R_3.25/lib/:$R_LIBS

      export R_LIBS=$R_LIBS

      for (( i=0; i < ${#ANATOMICAL_IMAGES[@]}; i++ ))
        do
          TEMPLATE_ORIGINATED_ANATOMICAL_IMAGE="${OUTPUT_DIRECTORY_FOR_SINGLE_SUBJECT_TEMPLATE}initTemplateAnatomical${i}.nii.gz"

          logCmd ${bindir}/originator.R --input ${ANATOMICAL_IMAGES[$i]} --template ${REGISTRATION_TEMPLATE} --output ${TEMPLATE_ORIGINATED_ANATOMICAL_IMAGE}

          TEMPLATE_ANATOMICAL_IMAGES="${TEMPLATE_ANATOMICAL_IMAGES} ${TEMPLATE_ORIGINATED_ANATOMICAL_IMAGE}"
        done
    else
      TEMPLATE_ANATOMICAL_IMAGES=${ANATOMICAL_IMAGES[@]}
    fi
...
for(( i=0; i < ${NUMBER_OF_MODALITIES}; i++ ))
  do
    ...
    logCmd ${ANTSPATH}/ImageMath 3 ${TEMPLATE_INPUT_IMAGE} PadImage ${TEMPLATE_ANATOMICAL_IMAGES[$i]} 5
...
logCmd rm -f ${OUTPUT_DIRECTORY_FOR_SINGLE_SUBJECT_TEMPLATE}initTemplateAnatomical*.nii.gz
  ...
  then
    ...
    if [[ $DO_REGISTRATION_TO_TEMPLATE -eq 0 ]];
      then
        ...
        if [[ $DO_COOK_BRAINEXTRACTION -eq 1 ]] && [[ ! -f ${SINGLE_SUBJECT_TEMPLATE_EXTRACTION_MASK} ]] ; 
          then
            # Phil's BE 
            COOK_BRAINEXTRACTION_SEGMENTATION_PRIOR=`echo ${SEGMENTATION_PRIOR} | sed 's/\%d/*/g'`
          logCmd ${bindir}/brainExtractionRegistration.pl \
            --input ${SINGLE_SUBJECT_TEMPLATE} \
            --template ${BRAIN_TEMPLATE} \
            --template-brain-mask ${REGISTRATION_TEMPLATE} \
            --template-reg-mask ${EXTRACTION_REGISTRATION_MASK} \
            --bias-correct 1 \
            ...
            --seg-priors ${COOK_BRAINEXTRACTION_SEGMENTATION_PRIOR} \
            --float 1 \
            --output-root ${SINGLE_SUBJECT_ANTSCT_PREFIX}_initBEreg

          logCmd ${bindir}/brainExtractionSegmentation.pl \
            --input ${SINGLE_SUBJECT_TEMPLATE} \
	    --initial-brain-mask ${SINGLE_SUBJECT_ANTSCT_PREFIX}_initBEregBrainMask.nii.gz \
            --bias-correct 1 \
            --erosion-radius 0 \
            --dilation-radius 2 \
            --include-initial-classes 4 5 6 \
            --output-root ${SINGLE_SUBJECT_ANTSCT_PREFIX}_kMeans_

           # logCmd cp ${SINGLE_SUBJECT_ANTSCT_PREFIX}_kMeans_BrainMask.nii.gz ${SINGLE_SUBJECT_TEMPLATE_EXTRACTION_MASK}
	   logCmd ${ANTSPATH}/CopyImageHeaderInformation ${SINGLE_SUBJECT_TEMPLATE} ${SINGLE_SUBJECT_ANTSCT_PREFIX}_kMeans_BrainMask.nii.gz ${SINGLE_SUBJECT_TEMPLATE_EXTRACTION_MASK} 1 1 1 
         #  logCmd rm ${SINGLE_SUBJECT_ANTSCT_PREFIX}_kMeans_BrainSegmentation.nii.gz

        #   logCmd rm ${SINGLE_SUBJECT_ANTSCT_PREFIX}_initBEreg*
...
# local clean-up
logCmd rm -f ${SINGLE_SUBJECT_ANTSCT_PREFIX}SubjectToTemplateLogJacobian.nii.gz
logCmd rm -f ${SINGLE_SUBJECT_ANTSCT_PREFIX}BrainNormalizedToTemplate.nii.gz
...
for (( i=0; i < ${#ANATOMICAL_IMAGES[@]}; i+=$NUMBER_OF_MODALITIES ))
  do
    ...
    if [[ $DO_COOK_BRAINEXTRACTION -eq 1 ]] && [[ ! -f ${OUTPUT_LOCAL_PREFIX}BrainExtractionMask.nii.gz ]] ; 
      then
        # Phil's BE 
        # fix the paths/files
        # "shortcut"
        logCmd ${bindir}/brainExtractionRegistration.pl \
          --input ${ANATOMICAL_IMAGES[$i]} \
          --template ${SINGLE_SUBJECT_TEMPLATE} \
          --template-brain-mask ${SINGLE_SUBJECT_TEMPLATE_EXTRACTION_MASK} \
          --template-reg-mask ${SINGLE_SUBJECT_TEMPLATE_EXTRACTION_REGISTRATION_MASK} \
          --bias-correct 1 \
          --laplacian-sigma 1 \
          --seg-priors ${SINGLE_SUBJECT_TEMPLATE_PRIORS} \
          --float 1 \
          --output-root ${OUTPUT_LOCAL_PREFIX}_initBEreg

        logCmd ${bindir}/brainExtractionSegmentation.pl \
          --input ${ANATOMICAL_IMAGES[$i]} \
          --initial-brain-mask ${OUTPUT_LOCAL_PREFIX}_initBEregBrainMask.nii.gz \
          --bias-correct 1 \
          --erosion-radius 0 \
          --dilation-radius 2 \
          --include-initial-classes 4 5 6 \
          --output-root ${OUTPUT_LOCAL_PREFIX}_kMeans_

        # logCmd cp ${OUTPUT_LOCAL_PREFIX}_kMeans_BrainMask.nii.gz ${OUTPUT_LOCAL_PREFIX}BrainExtractionMask.nii.gz
	logCmd ${ANTSPATH}/CopyImageHeaderInformation ${ANATOMICAL_IMAGES[$i]} ${OUTPUT_LOCAL_PREFIX}_kMeans_BrainMask.nii.gz ${OUTPUT_LOCAL_PREFIX}BrainExtractionMask.nii.gz 1 1 1 
       # logCmd rm ${OUTPUT_LOCAL_PREFIX}_kMeans_BrainSegmentation.nii.gz

        logCmd rm ${OUTPUT_LOCAL_PREFIX}_initBEreg*
