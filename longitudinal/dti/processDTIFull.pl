...
my $outputBaseDir = "/data/grossman/pipedream2017/longitudinal/dti/";

my $antsLongCT_BaseDir = "/data/grossman/pipedream2017/longitudinal/antsLongCT_2";

my $antsPath = "/data/grossman/pipedream2017/bin/ants/bin/";

my $caminoDir = "/data/grossman/pipedream2017/bin/camino/bin";

my $acqParamsDir = "/data/grossman/pipedream2017/metadata";
...
for ( my $i=0; $i < $numTimepoints; $i++ ) {
    ...
    my $sst = "${antsCTOutputRoot}ExtractedBrain0N4.nii.gz";
    my $sstMask = "${antsCTOutputRoot}BrainExtractionMask.nii.gz";
...
${Bin}/processScanDTI.pl --input-dir $inputDWI_Dir --output-dir $outputTP_Dir --output-file-root ${subject}_${tp}_ --acq-params $acqParams --eddy-correct-method $eddyCorrectMethod --antsct-output-root ${antsCTOutputRoot} --template $sst --template-mask $sstMask
